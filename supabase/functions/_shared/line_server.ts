  ...
  if (bookings.length === 0) {
    ...
    if (buttons && buttons.length > 0) {
      flex.contents["footer"] = {
        ...
        "layout": "vertical",
        "spacing": "sm",
        "contents": buttons // 這裡的 buttons 已經是陣列了
      };
    }
    return flex;
  }
...
function createBookingFlex_(searchData, procedureData) {
  const bookings = procedureData || [];
  if (bookings.length === 0) {
    const buttons = createButtons_(searchData.no_data_keys);
    const flex = {
    ...
    bubbles = bookings.map((b) => {
      ...
      const flex = {
        ...
        "header": {
          "type": "box",
          "layout": "vertical",
          "backgroundColor": canCancel ? "#D9534F" : '#F59E0B',
          "contents": [
            {
              "type": "text",
              "text": canCancel ? "選擇取消這筆預約" : "正在申請取消中",
              "color": "#FFFFFF",
              "weight": "bold",
              "size": "md"
            }
...
//還需要其他服務嗎
function createQuickActionsFlex_(more_keys = []) {
